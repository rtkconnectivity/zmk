...
bt_addr_le_t *zmk_ble_active_profile_addr(void) { 
	...
	// }
    LOG_ERR("print active profile value %d address 0x%x, value of original address%d", active_profile, &active_profile,*(uint8_t *)0x20b061);
    ...
    LOG_ERR("test_flag static value %d, addr 0x%x", test_flag, &test_flag);
    // DBG_DIRECT("test_flag static value %d, addr 0x%x", test_flag, &test_flag);
...
static void zmk_check_profile_exist_static_random_addr(bool need_gen_new_addr) {
    ...
    LOG_DBG("Loaded %s address for cur profile %d, need generate new addr(1:true, 0:false): %d",
            addr_str, active_profile, need_gen_new_addr);
