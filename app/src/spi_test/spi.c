...
void test_spi_loopback(void)
{
	...
	LOG_ERR("spi dev slow cs gpio port %s, pin %d",spi_slow.config.cs.gpio.port->name, spi_slow.config.cs.gpio.pin);
	...
	LOG_ERR("spi dev fast cs gpio port %s, pin %d",spi_fast.config.cs.gpio.port->name, spi_fast.config.cs.gpio.pin);
